...
namespace EuchreSim
{
    public class PlayerRed : IPlayer
    {
        ...
        public PlayerRed(string name, Team team, byte Id)
        {
            Name = name;
            Team = team;
            ID = Id;
        ...
        {
            var badCards = _hand.Where(c => c.Rank == Rank.Nine || c.Rank == Rank.Ten).Take(3);
        ...
        public CallResponse CallIt(Card upCard, Position dealer)
        {
            ...
            var score = GetHandScore(upCard.Suit);
            var result = score > 23 ? CallResponse.CallIt : CallResponse.Pass;
            //Console.WriteLine($"{Name} callit:{result} score:{score} up{upCard}");
        ...
        {
            Console.WriteLine($"{Name} First:{_firstPlayerCount} {postionCount[Position.Dealer]}-{postionCount[Position.LeftOfDealer]}-{postionCount[Position.DealerPartner]}-{postionCount[Position.RightOfDealer]}");
        }
    }
}
